...
class Func {
    ...
    public static function exportExcel($expTitle, $expCellName, $expTableData, $topData, $otherData) {
        ...
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('A2', $otherData);
            ...
            for ($j = 0; $j < $cellNum; $j++) {
                if ($expCellName[$j][0] == 'account_type') {
                    if ($expTableData[$i][$expCellName[$j][0]] == 0) {
                        $objPHPExcel->getActiveSheet(0)->setCellValue($cellName[$j] . ($i + $topNum + 4), '餐饮');
                    } else {
                        $objPHPExcel->getActiveSheet(0)->setCellValue($cellName[$j] . ($i + $topNum + 4), '果蔬');
                    }
                } else {
